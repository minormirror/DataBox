...
function InfoBox(bc_){
  ...
  this.id = 0;                     //eindeutige Id für den aktuellen Zustand der Infobox: eine eindeutige Referenz für x,y,widht,height,content dieser IB
...
//  -----------------------------------------------------------------------------------------------------------------------
 /* var mom_width = unpx(this.div_element.style.width);
  var mom_height = unpx(this.div_element.style.height);
  var widthHalf = (mom_width+1)/2;      
  var heightHalf = (mom_height+1)/2;
  this.div_element.style.left = (window.pageYOffset + (window.innerWidth/2) - widthHalf)+"px";
  this.div_element.style.top = (window.pageYOffset + (window.innerHeight/2) - heightHalf)+"px";
  alert (widthHalf +" "+ heightHalf);*/
  ...
  this.div_element_onmousedown = function(ev){ 
    //return this.bc.eventSwitch("INFO_BOX_MOUSEDOWN",this,ev);
	this.bc.setFocus(this.div_element);
	return true;
  }
  ...
  img2.onmousedown = this.img2onclick.bindAsEventListener(this);
