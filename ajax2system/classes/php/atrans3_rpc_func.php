...
include_once("rpc-server-php/RPCServer.class.php4");
...
function getChangeFunc2Content($project_id,$f2hIds){ //Position zu einer Infobox hinzufügen
  ...
  while ($line = mysql_fetch_array($result, MYSQL_ASSOC)){    	
    $pos_data = array();
	$pos_data["id"] = $f2hIds[$line["subversion"]];
	$pos_data["x"] = $line["x"];
	$pos_data["y"] = $line["y"];
	$pos_data["w"] = $line["width"];
	$pos_data["h"] = $line["height"];
	
	array_push($res,$pos_data);    
  }
...
function getChangeFunc3Content($project_id,$f3hIds){ //Inhalt zu einer Infobox hinzufügen
  ...
  while ($line = mysql_fetch_array($result, MYSQL_ASSOC)){
    $new_cont = array();	
	$new_cont["id"] = $f3hIds[$line["subversion"]];
	$new_cont["content"] = $line["content"];
    array_push($res,$new_cont);	
  }
...
function rpc_update($project_id,$subversion){
  $res = array();  
  $error = 0;
  ...
  //add max subversion 
  $res["max_subversion"] = $maxsubversion;
  ...
  //add f1 createIds:  
  $res["create_id"] = getChangeFunc1Content($element_id,$change_fct,$delete_ids);
	  
  //add f2 Position einer Infobox
  if (sizeof($f2hIds) != 0){
    $res["update_pos"] = getChangeFunc2Content($project_id, $f2hIds);	  
  ...
  //add f3 new content
  if (sizeof($f3hIds) != 0){
    $res["update_content"] = getChangeFunc3Content($project_id, $f3hIds);	  
  ...
  //add f4 delIds:
  $res["remove_ids"] = getChangeFunc4Content($element_id,$change_fct,$delete_ids);   
  
  return $res;
}

function rpc_createIb($project_id,$rand_id){
...
function rpc_updatePos($project_id,$ib_id,$x,$y,$width,$height){
  ...
  $result=mysql_query("INSERT INTO ib_ib_pos (project_id, subversion, x,y,width,height) VALUES". 
                        "('".$project_id."', '".$newsubversion."', '".$x ."', '".$y ."', '".$width ."', '".$height ."');");
  ...
  $result=mysql_query("INSERT INTO ib_change (project_id, subversion, element_id, change_fct) VALUES". 
                        "('".$project_id."', '".$newsubversion."', '".$ib_id ."', '2');");     
  if (!$result){
    $error = "-4005";      
  }
...
function rpc_updateText($project_id,$ib_id,$content){
  ...
  $result=mysql_query("INSERT INTO ib_ib_content (project_id, subversion, content) VALUES". 
                        "('".$project_id."', '".$newsubversion."', '".$content ."');");
  ...
  $result=mysql_query("INSERT INTO ib_change (project_id, subversion, element_id, change_fct) VALUES". 
                        "('".$project_id."', '".$newsubversion."', '".$ib_id ."', '3');");
...
$server->addMethod("rpc_test");
$server->addMethod("rpc_update");
$server->addMethod("rpc_createIb");
$server->addMethod("rpc_updatePos");
$server->addMethod("rpc_updateText");
